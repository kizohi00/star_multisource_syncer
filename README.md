...
- يمرر العامل حالة كل المصادر (`azorafly` و`3asq` و`mangatek` و`sparkmanga` و`mangaswat` و`teamx`) عبر دالة التطبيع المشتركة في [`src/mangastar_multisource/domain/status.py`](src/mangastar_multisource/domain/status.py). تُحوَّل القيم العربية والإنجليزية الموثقة إلى `ongoing` أو `completed` أو `hiatus`، وتُهمل القيم غير المعروفة أو `coming` دون تخمين. عند وجود حالة مؤكدة وعملٍ مرتبط بسلسلة، يحدّث `series.story_status` إذا اختلفت القيمة الحالية. وعند إنشاء سلسلة جديدة، تُكتب الحالة نفسها أثناء الإدراج. لا يغيّر العامل `series.translation_status`؛ هذا العمود مهجور في التصميم الحالي. تفاصيل الحقول والأدلة في [`docs/STATUS_NORMALIZATION.md`](docs/STATUS_NORMALIZATION.md).
