...
- يترجم العامل حالات Manga Peak العربية إلى القيم القياسية قبل التحديث: `مستمر`/`مستمرة` → `ongoing`، `مكتمل`/`مكتملة`/`منتهي`/`منتهية` → `completed`، و`متوقف`/`متوقفة`/`في استراحة` → `hiatus`. عند وجود حالة مصدر مؤكدة وعملٍ مرتبط بسلسلة، يحدّث `series.story_status` إذا اختلفت القيمة الحالية. لا يغيّر العامل `series.translation_status`؛ هذا العمود مهجور في التصميم الحالي.
