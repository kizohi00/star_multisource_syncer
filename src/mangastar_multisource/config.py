...
def _default_sync_database_name(database_name: str) -> str:
    """Choose a separate same-server schema when no override is supplied."""
    if database_name.endswith("_copy"):
        return f"{database_name[:-5].rstrip('_')}_sync_copy"
    return f"{database_name}_sync"
...
class Settings:
    ...
    # The sync metadata lives in a separate database on the same MariaDB
    # server. The optional name preserves compatibility with direct Settings
    # construction in tests; from_env always resolves an explicit name.
    sync_db_name: str | None = None

    @property
    def effective_sync_db_name(self) -> str:
        return self.sync_db_name or _default_sync_database_name(self.db_name)
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            sync_db_name=os.getenv("MANGA_SYNC_DB_NAME") or _default_sync_database_name(db_name),
        )
    ...
    def validate(self, *, require_password: bool = True) -> None:
        ...
        if self.effective_sync_db_name == self.db_name:
            raise ValueError(
                "MANGA_SYNC_DB_NAME must name a separate database; it cannot equal MANGA_DB_NAME."
            )
        ...
        if (
            not self.effective_sync_db_name.endswith("_copy")
            and os.getenv("MANGA_ALLOW_NON_COPY_DB") != "1"
        ):
            raise ValueError(
                f"Refusing sync database {self.effective_sync_db_name!r}. "
                "Use a *_copy sync database for testing, or explicitly set "
                "MANGA_ALLOW_NON_COPY_DB=1 for the approved deployment."
            )
